...
func QueryList(db *database.DB, tpl *Template, offset, limit int, args ...interface{}) (int, []string, [][]interface{}, error) {
...
func QueryMap(db *database.DB, tpl *Template, offset, limit int, args ...interface{}) (int, []map[string]interface{}, error) {
	count, err := QueryCount(db, tpl.CountSql, args)
	if err != nil {
...
// 执行一个通用的计数
func QueryCount(db *database.DB, querySql string, args ...interface{}) (int, error) {
	if len(querySql) == 0 {
		return 1, nil
	}
	count := 0
...
// 执行一个通用的查询
func QueryData(db *database.DB, querySql string, args ...interface{}) (titles []string, result [][]interface{}, err error) {
	titles = []string{}
	result = [][]interface{}{}
	rows, err := db.Query(querySql, args...)
	if err != nil {
...
func QueryOne(db *database.DB, querySql string, args ...interface{}) (map[string]interface{}, error) {
	rows, err := db.Query(querySql, args...)
	if err != nil {
		if strings.Index(err.Error(), "Error 1146") != -1 {
			// no table
