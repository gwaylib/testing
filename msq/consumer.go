...
type HandleContext func(ctx context.Context, job *beans.Job, tried int) bool
...
func (c *worker) connect() error {
	...
	conn, _ := beans.NewConn(kon, c.addr)
	c.conn = conn
...
func (c *worker) do(job *beans.Job) error {
	...
	go func(ctx context.Context) {
		...
		defer func() {
			// recover for handle
			if r := recover(); r != nil {
				c.log.Error(errors.New("handle panic").As(r))
				debug.PrintStack()
				time.Sleep(10e9)
				deal = false
			}
...
func (c *worker) nextTry(job *beans.Job) {
	id := job.ID
...
func (c *worker) delJob(job *beans.Job) {
	id := job.ID
